import { $a as RGFormat, Ar as LinearMipmapLinearFilter, B as BufferAttribute, Er as LinearFilter, Fs as Vector4, Gr as MathUtils, Hr as Material, Ht as DepthTexture, Ji as PerspectiveCamera, Jr as Matrix4, Ko as StaticDrawUsage, Kr as Matrix2, Mn as HalfFloatType, Mr as LinearSRGBColorSpace, Ni as Object3D, No as Sphere, Ns as Vector2, Nt as DataArrayTexture, Or as LinearMipMapLinearFilter, Ps as Vector3, Pt as DataTexture, Q as ClampToEdgeWrapping, Qt as DynamicDrawUsage, R as BoxGeometry, Un as InstancedInterleavedBuffer, Us as WebGPUCoordinateSystem, V as BufferGeometry, Vn as InstancedBufferAttribute, Vs as WebGLCoordinateSystem, Wi as OrthographicCamera, Xn as InterleavedBufferAttribute, Xr as Mesh, Yi as Plane, Yn as InterleavedBuffer, Zr as MeshBasicMaterial, _n as FramebufferTexture, cc as warn, cn as EventDispatcher, co as RenderTarget, ec as error, et as Color, fi as NearestFilter, gt as CubeTexture, ic as log$2, is as Texture, lc as warnOnce, mn as FloatType, nt as ColorManagement, pn as Float32BufferAttribute, pt as CubeDepthTexture, qr as Matrix3, rt as Compatibility, sn as Euler, ya as RGBAFormat } from "./three.core-wVQK10Or.js";
	...
	*/
	deserialize(json) {
		if (json.inputNodes !== void 0) {
			...
			} else this[property] = nodes[json.inputNodes[property]];
		}
	}
	/**
	...
	*/
	...
	setup(builder) {
		...
		if (normalMapType === 0) {
			...
			else if (unpackNormalMode !== "") console.error(`THREE.NodeMaterial: Unexpected unpack normal mode: ${unpackNormalMode}`);
		} else if (unpackNormalMode !== "") console.error(`THREE.NodeMaterial: Normal map type '${normalMapType}' is not compatible with unpack normal mode '${unpackNormalMode}'`);
		if (scaleNode !== null) {
			let scale = scaleNode;
...
var NodeMaterial = class extends Material {
	...
	*/
	setup(builder) {
		...
		const renderTarget = renderer.getRenderTarget();
		if (renderer.contextNode.isContextNode === true) builder.context = {
			...builder.context,
			...renderer.contextNode.getFlowContextData()
		};
		else error("NodeMaterial: \"renderer.contextNode\" must be an instance of `context()`.");
		if (this.contextNode !== null) if (this.contextNode.isContextNode === true) builder.context = {
			...builder.context,
			...this.contextNode.getFlowContextData()
		};
		else error("NodeMaterial: \"material.contextNode\" must be an instance of `context()`.");
		builder.addStack();
...
var StackNode = class extends Node {
	...
	*/
	If(boolNode, method) {
		this._currentCond = select$1(boolNode, new ShaderNode$1(method));
...
var backgroundRotation$1 = /* @__PURE__ */ uniform$1(new Matrix4()).setGroup(renderGroup$1).onRenderUpdate(({ scene }) => {
	const background = scene.background;
	if (background !== null && background.isTexture && background.mapping !== 300) {
...
var normal = Fn$1(({ texture, uv }) => {
	...
	If$1(uv.x.lessThan(epsilon), () => {
		...
		const step = .01;
		const x = texture.sample(uv.add(vec3$1(-step, 0, 0))).r.sub(texture.sample(uv.add(vec3$1(step, 0, 0))).r);
		const y = texture.sample(uv.add(vec3$1(0, -step, 0))).r.sub(texture.sample(uv.add(vec3$1(0, step, 0))).r);
		const z = texture.sample(uv.add(vec3$1(0, 0, -step))).r.sub(texture.sample(uv.add(vec3$1(0, 0, step))).r);
	...
	setupLightsNode(builder) {
		...
		for (const light of lights) if (light.isNode) lightNodes.push(nodeObject$1(light));
			...
			if (lightNode === null) {
				...
				} else lightNode = _lightsNodeRef.get(light);
